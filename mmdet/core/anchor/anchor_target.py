...
def anchor_target_single(flat_anchors,
    ...
    num_level_anchors_inside = \
        get_num_level_anchors_inside(num_level_anchors,
                                     inside_flags)
    ...
    if sampling:
        ...
        # if len(ext_data) > 0:
            if "GFL" in cfg.assigner.get("type"):
                assign_result = bbox_assigner.assign(anchors, num_level_anchors_inside,
                                                     ext_data, bbox_coder, gt_bboxes,
                                                     gt_bboxes_ignore, gt_labels,
                                                     img_meta)
            else:
                assign_result = bbox_assigner.assign(anchors, ext_data, bbox_coder,
                                                     gt_bboxes, gt_bboxes_ignore,
                                                     gt_labels, img_meta)

        else:
            if "ATSS" in cfg.assigner.get("type"):
                assign_result = bbox_assigner.assign(anchors, num_level_anchors_inside,
    ...
    if len(pos_inds) > 0:
        ...
        if not hasattr(bbox_assigner, 'weight'):
            ...
            bbox_weights[pos_inds, :] = bbox_assigner.weight['bbox_weight']
            if gt_labels is None:
                labels[pos_inds] = 1
            else:
                labels[pos_inds] = gt_labels[sampling_result.pos_assigned_gt_inds]
            label_weights = bbox_assigner.weight['cls_weight']

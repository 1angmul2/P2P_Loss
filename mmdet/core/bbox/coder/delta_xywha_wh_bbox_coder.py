...
def norm_angle(rbboxes, ag_range=(-np.pi/4, np.pi)):
    # angle range -> [-45, 135)
    angle = (rbboxes[:, 4:5] - ag_range[0]
             ) % ag_range[1] + ag_range[0]
    return torch.cat([rbboxes[:, :4], angle],
                     dim=1)
...
class DeltaXYWHAWHBBoxCoder(BaseBBoxCoder):
    ...
    def encode(self, bboxes, gt_bboxes):
        ...
        encoded_bboxes = bbox2delta_rotated(bboxes, gt_bboxes, self.means,
                                            self.stds)

...
def pts_in_rect(pts, rbboxes):
    """
    :param pts: [m, n, 2]
    :param rbboxes: [n, 5]
    :return:
    """
    if rbboxes.shape[1] == 6:
        rbboxes = rbboxes[:, :5]

    # [n,5] -> [n, 4, 2]
    bbox_pts = rbbox2points(rbboxes).view(-1, 4, 2)

    # [m, n, 1, 2]
    pts = pts[:, :, None, :]
    # [1, n, 4, 2]
    bbox_pts = bbox_pts[None]
    bbox_pts_roll = torch.roll(bbox_pts, -1, 2)
    v_tt = bbox_pts_roll - bbox_pts
    ...
    # [m, n, 4, 3] -> [m, n, 4, 3] -> [m, n, 4]
    cross = torch.cross(v_tp, v_tt, dim=3).sum(-1)

    pts_in_tri = torch.logical_xor(
        cross.gt(0).any(dim=2),
        cross.lt(0).any(dim=2))
    return pts_in_tri


def points2minbbox(pts, xyxy_mode=False):
    assert pts.shape[1] == 8
    pts = pts.view(-1, 4, 2)
    xy_min = pts.min(dim=1)[0]
    xy_max = pts.max(dim=1)[0]

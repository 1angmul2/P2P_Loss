...
@BBOX_ASSIGNERS.register_module
class RATSSAssigner(ATSSAssigner):
    """Assign a corresponding gt bbox or background to each bbox.
    ...
    def __init__(self, use_hbbox=False, **kwargs):
        ...
        super(RATSSAssigner, self).__init__(**kwargs)
        if use_hbbox:
    ...
    def assign(self,
        ...
        if self.use_hbbox:
            bboxes = points2minbbox(rbbox2points(bboxes[:, :5]),
                                    xyxy_mode=True)
            gt_bboxes = points2minbbox(rbbox2points(gt_bboxes[:, :5]),
                                       xyxy_mode=True)
            return super(RATSSAssigner, self).assign(
        ...
        ep_pts = bboxes_points.view(
            1, -1, 2).expand(
            num_gt, -1, -1
        ).contiguous().view(-1, 2)[candidate_idxs]

        # calculate the left, top, right, bottom distance between positive
        # bbox center and gt side
        is_in_gts = pts_in_rect(
            ep_pts.view(-1, num_gt, 2),
            gt_bboxes)
        del ep_pts
        is_pos = is_pos & is_in_gts
